...
public class GreedyChooseMeta {
    public int KSIZE;
    public TrajectoryMeta[] MaxHeap; //所有的k个轨迹
    public static int index = 1; //当前的末尾index,下一个index

    public GreedyChooseMeta(int KSize) {
        KSIZE = KSize;
        MaxHeap = new TrajectoryMeta[KSize + 1];

        double MAXVALUE = Double.MAX_VALUE;
        MaxHeap[0] = new TrajectoryMeta(MAXVALUE);
        index = 1;
    }
    ...
    public void addNewTraj(TrajectoryMeta traj) {
        MaxHeap[index] = traj;
        orderAdjust(index);
        index++;
    }
    ...
    private void swag(int index1, int index2) {
        TrajectoryMeta temp = MaxHeap[index1];
        MaxHeap[index1] = MaxHeap[index2];
        MaxHeap[index2] = temp;
    }

    public TrajectoryMeta getHeapHead() {
        return MaxHeap[1];
    }

    public TrajectoryMeta getMaxScoreTraj() {
        index--;
        TrajectoryMeta temp = MaxHeap[1];
